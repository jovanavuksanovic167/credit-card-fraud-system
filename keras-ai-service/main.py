...
DATASET_PATH = (
    Path(__file__).resolve().parent.parent / "datasets" / "card_transdata.csv"
)
...
BINARY_FEATURES = ["repeatRetailer", "usedChip", "usedPinNumber", "onlineOrder"]
...
app = FastAPI(title="Credit Card Fraud AI Service", lifespan=lifespan)
...
def predict_transaction(request: FraudPredictionRequest):
    transaction = pd.DataFrame(
        [
            {
                "amount": request.amount,
                "distanceFromHome": request.distanceFromHome,
                "distanceFromLastTransaction": request.distanceFromLastTransaction,
                "ratioToMedianPurchasePrice": request.ratioToMedianPurchasePrice,
                "repeatRetailer": request.repeatRetailer,
                "usedChip": request.usedChip,
                "usedPinNumber": request.usedPinNumber,
                "onlineOrder": request.onlineOrder,
            }
        ]
    )

    processed_transaction = preprocess_for_prediction(transaction)

    with model_lock:
        probability = float(
            model.predict(processed_transaction, verbose=0).flatten()[0]
        )
        threshold = float(model_config["threshold"])

    probability_percent = round(probability * 100, 6)
    is_fraud = probability >= threshold

    return FraudPredictionResponse(
        fraudProbabilityPercent=probability_percent, fraud=is_fraud
    )
...
def process_daily_transactions(count: int = Query(default=1000, ge=100, le=10000)):
    ...
    metrics = calculate_metrics(
        y_true=y_true, probabilities=probabilities, threshold=threshold
    )
...
def prepare_train_test_data_for_retraining(data: pd.DataFrame):
    ...
    X_train_processed[NUMERIC_FEATURES] = np.log1p(X_train_processed[NUMERIC_FEATURES])

    X_test_processed[NUMERIC_FEATURES] = np.log1p(X_test_processed[NUMERIC_FEATURES])
...
def retrain_model():
    ...
    X_train, X_test, y_train, y_test, new_scaler = (
        prepare_train_test_data_for_retraining(data)
    )
    ...
    metrics = calculate_metrics(
        y_true=y_test, probabilities=probabilities, threshold=0.80
    )
